...
預設`admin`用戶帳戶用於驗證預設安裝的複製代理。 您應建立專用用戶帳戶以用於複製代理。

如需詳細資訊，請參閱AEM安全性檢查清單的「設定複製與傳輸使用者[」一節。](https://helpx.adobe.com/experience-manager/6-3/sites/administering/using/security-checklist.html#VerificationSteps)
...
1. 按一下「編輯」，並在「設定」標籤中確定已選取&#x200B;**Enabled**。

1. （可選）要啟用別名或虛名路徑失效請求，請選擇&#x200B;**別名更新**&#x200B;選項。
1. 在「傳輸」頁籤上，輸入訪問Dispatcher所需的URI。\
   ...
   **注意：對** 於Dispatcher Flush代理，只有在使用基於路徑的虛擬主機條目來區分場時，才使用URI屬性。使用此欄位可以定位要失效的場。 例如，場#1的虛擬主機為`www.mysite.com/path1/*` ，場#2的虛擬主機為`www.mysite.com/path2/*`。 您可以使用`/path1/invalidate.cache`的URL來定位第一個群，使用`/path2/invalidate.cache`來定位第二個群。 有關詳細資訊，請參閱[將Dispatcher與多個域一起使用](dispatcher-domains.md)。

1. 視需要設定其他參數。
1. 按一下「確定」激活代理。

或者，您也可以從[AEM Touch UI](https://helpx.adobe.com/experience-manager/6-2/sites/deploying/using/replication.html#ConfiguringaDispatcherFlushagent)存取和設定Dispatcher Flush代理。

如需如何啟用虛名URL存取權的詳細資訊，請參閱[啟用虛名URL存取權](dispatcher-configuration.md#enabling-access-to-vanity-urls-vanity-urls)。
...
## 從發佈實例{#invalidating-dispatcher-cache-from-a-publishing-instance}中使Dispatcher快取失效
...
* 防止Dispatcher和發佈實例之間可能的定時衝突（請參閱「編寫環境」中的「使Dispatcher快取無效」[）。](#invalidating-dispatcher-cache-from-the-authoring-environment)
...
## 手動使Dispatcher Cache {#manually-invalidating-the-dispatcher-cache}失效

要使Dispatcher快取無效（或刷新）而不激活頁，可以向Dispatcher發出HTTP請求。 例如，您可以建立AEM應用程式，讓管理員或其他應用程式清除快取。
...
Dispatcher會刷新（刪除）名稱與`CQ-Handler`標題值相匹配的快取檔案和資料夾。 例如，`/content/geomtrixx-outdoors/en`的`CQ-Handle`符合下列項目：

* `geometrixx-outdoors`目錄中名為`en`的所有檔案（任何副檔名）

* en目錄下名為&quot; `_jcr_content`&quot;的任何目錄（如果存在，則包含頁面子節點的快取轉換）

通過觸摸`.stat`檔案，調度器快取中的所有其他檔案（或最高到特定級別，取決於`/statfileslevel`設定）都將失效。 此檔案的上次修改日期與快取文檔的上次修改日期進行比較，如果`.stat`檔案較新，則重新提取該文檔。 有關詳細資訊，請參閱[按資料夾級別驗證檔案](dispatcher-configuration.md#main-pars_title_26)。

通過發送附加的標題`CQ-Action-Scope: ResourceOnly`，可以防止失效（即。stat檔案的觸摸）。 這可用於刷新特定資源而不使快取的其他部分失效，例如動態建立並需要定期刷新的JSON資料（例如，表示從第三方系統獲取的資料以顯示新聞、股票行情等）。
...
**注意：** 僅應在發佈例項上執行刪除和重新存檔檔案。從作者實例執行時，當重新快取資源的嘗試在發佈之前發生時，就會發生競爭條件。
...
要立即重新快取的頁面路徑會列在消息正文的單獨行上。 `CQ-Handle`的值是使要重新快取的頁面無效的頁面路徑。 （請參閱[Cache](dispatcher-configuration.md#main-pars_146_44_0010)配置項的`/statfileslevel`參數。） 以下示例HTTP請求消息刪除並重新快取`/content/geometrixx-outdoors/en.html page`:
...
以下代碼實現了向Dispatcher發送無效請求的servlet。 Servlet接收包含`handle`和`page`參數的請求消息。 這些參數分別提供`CQ-Handle`標題的值和要重新快取的頁面路徑。 Servlet使用這些值來構建Dispatcher的HTTP請求。
