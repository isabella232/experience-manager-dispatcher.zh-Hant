...
透過Apache-2.4提出的請求 `mod_ssl` 包含類似下列範例的標題：
...
* 處理HTTPS請求的虛擬主機。
* 虛 `renders` 擬主機的區段包含識別使用HTTPS之CQ或AEM例項之主機名稱和連接埠的項目。
* 項 `renders` 目包含名為value `secure` 的屬性 `1`。

注意：如果需要，請建立另一個虛擬主機以處理HTTP請求。

以下示例dispatcher.any檔案顯示了使用HTTPS連接到在主機和埠上運行的CQ實例的 `localhost` 屬性值 `8443`:
...
## 在Dispatcher和AEM之間設定互相SSL {#configuring-mutual-ssl-between-dispatcher-and-aem}
...
1. [為您的平台安裝](dispatcher-install.md) 最新版本的Dispatcher。 使用支援SSL的Dispatcher二進位檔案（SSL在檔案名中，如dispatcher-apache2.4-linux-x86-64-ssl10-4.1.7.tar）。
1. [為Dispatcher和render實例建立或獲取](dispatcher-ssl.md#main-pars-title-3) CA簽名證書。
1. [建立包含轉譯憑證的金鑰庫](dispatcher-ssl.md#main-pars-title-6) ，並設定轉譯的HTTP服務以使用它。
1. [為相互SSL配置Dispatcher Web](dispatcher-ssl.md#main-pars-title-4) server模組。
...
### 在Render電腦上配置SSL {#configuring-ssl-on-the-render-computer}
...
將調度器憑證和未加密的私密金鑰結合為單一PEM檔案。 使用文本編輯器或 `cat` 命令建立類似以下示例的檔案：
...
### 指定要用於Dispatcher的證書 {#specifying-the-certificate-to-use-for-dispatcher}

將以下屬性添加到 [Dispatcher模組配置](dispatcher-install.md#main-pars-55-35-1022) (在 `httpd.conf`):
